...
frontend_root="${games_root}/frontend"
backend_root="${games_root}/backend"
platform_root="${games_root}/platform"
mkdir -p "${frontend_root}/build" "${backend_root}/scripts" \
    "${backend_root}/src" "${platform_root}/platform" \
    "${platform_root}/docs/projects/games"
cp "${subject_script}" "${backend_root}/scripts/package-games-sources.sh"
printf 'frontend\n' > "${frontend_root}/tracked.txt"
printf 'backend\n' > "${backend_root}/src/tracked.txt"
printf 'platform\n' > "${platform_root}/platform/tracked.txt"
printf 'memory\n' > "${platform_root}/docs/projects/games/agent-memory.md"
printf 'ignored build\n' > "${frontend_root}/build/generated.txt"
printf '/build/\n' > "${frontend_root}/.gitignore"

for repository in "${frontend_root}" "${backend_root}" "${platform_root}"; do
    git -C "${repository}" init -q
done
git -C "${frontend_root}" add -- .gitignore tracked.txt
git -C "${backend_root}" add -- scripts/package-games-sources.sh src/tracked.txt
...
printf 'new frontend\n' > "${frontend_root}/new.js"
printf 'new backend\n' > "${backend_root}/src/new.txt"
printf 'new platform\n' > "${platform_root}/platform/new.txt"

package_script="${backend_root}/scripts/package-games-sources.sh"
