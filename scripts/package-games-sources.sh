...
product_root=$(CDPATH= cd -- "${backend_root}/.." && pwd)
frontend_root="${product_root}/frontend"
...
explicit_source() {
    ...
    case ${archive_path} in
        games/*)
            case ${archive_path} in
                games/frontend/*)
                    explicit_repository=${frontend_root}
                    explicit_relative_path=${archive_path#games/frontend/}
                    ;;
                games/backend/*)
                ...
                games/platform/*)
                    ...
                    ;;
                *) fail "explicit path must identify a Games repository: ${archive_path}" ;;
            esac
            ;;
        *)
...
git -C "${frontend_root}" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
    || fail "Games frontend is not a Git repository: ${frontend_root}"
git -C "${backend_root}" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
    || fail "Games backend is not a Git repository: ${backend_root}"
git -C "${platform_root}" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
    || fail "Games Platform checkout is not a Git repository: ${platform_root}"
...
stage_git_selection "${frontend_root}" 'games/frontend' '.'
